...
## 起動軽量化後の再計測（2026-09-03）

`982f13b`反映後、外部アクセスを15分以上停止してから本番URLへ最初のHTTPアクセスを実施した。初回のcurlは約70.97秒後にHTTP 000（peer側終了）となり、正常なHTMLは受け取れなかった。その直後の再アクセスではHTTP 200、TTFB 13.086秒、合計14.332秒、ボディ非空を確認した。Render公式が示すFreeサービスの復帰待ちが残るため、今回のアプリ側軽量化で待ち時間が完全には解消されないことを確認した。初回接続はエッジ側の接続終了を含むため、単一回の秒数を厳密な起動時間とは扱わず、実運用上は「15分後の初回アクセスでは数十秒の待機が発生し得る」と記録する。

その後の購入者QR入口では、件数チップなし、注文タイミング注意書き、入力前チェック、受け取り手順、広告画像の表示を確認した。

## 携帯電話型キーボードの複数文字入力対策（2026-09-04）

購入者ページの入力欄で、1回のキー操作が`ABC`のように複数文字として届くケースに対応するため、入力値から最初の英字1文字を採用し、その後の数字だけを最大3桁保持する`normalizeTicketCodeInput`を追加した。これにより、`ABC`は`A`、`ABC123`は`A123`、`A-123`は`A123`となり、数字だけ・記号だけの入力は空欄になる。完全なチケット番号の貼り付けにも対応する。

入力正規化のVitestを追加し、携帯電話型キーの複数英字、完全な貼り付け、ノイズ混在、数字／記号のみを検証した。Vitestは6ファイル・15テスト、型チェック、本番ビルドに成功した。購入者ページは375×812のフルページ表示で、注意事項を読んだ後に入力欄へ進むレイアウトと件数非表示が維持されていることを確認した。
